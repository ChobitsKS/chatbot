...
//   const answer = askOpenAI({ userMessage, history });
...
function askOpenAI(params = {}) {
  ...
  const maxOutputTokens = Number(
    getConfig("OPENAI_MAX_OUTPUT_TOKENS", "700")
  );
  ...
  const history = Array.isArray(params.history) ? params.history : [];
  /*
   * สำหรับคำถามข้อมูลจริง ไม่ควรให้ประวัติเก่ามีอิทธิพล
   * มากกว่าคำถามล่าสุดและเอกสารอ้างอิง
   */
  ...
  const inputText = [
    historyText
      ? "ประวัติการสนทนา ใช้เป็นบริบทประกอบเท่านั้น:\n" + historyText
      : "",
  ...
  /*
   * ให้ OpenAI ค้น Vector Store เอง
   * แทนการเรียก /vector_stores/{id}/search แยกก่อน
   */
  if (vectorStoreId) {
    payload.tools = [
      {
        ...
        max_num_results: Number(
          getConfig("VECTOR_MAX_RESULTS", "8")
        )
      }
    ];
  }
...
function buildVectorContext(query) {
  ...
  const maxResults = Number(getConfig("VECTOR_MAX_RESULTS", "10"));
...
function testVectorSearch() {
  const query = "แผนภาษาอังกฤษและดิจิทัล เรียนภาคกลาง ทะเบียนบ้านอยู่หาดใหญ่สมัครได้ไหมครับ";
  const result = buildVectorContext(query);
  Logger.log("========== QUERY ==========");
  Logger.log(query);
  Logger.log("========== VECTOR CONTEXT ==========");
  Logger.log(result);
  Logger.log("========== END ==========");
}
...
function testChatReply() {
  const userMessage = "สวัสดีครับ แผนภาษาอังกฤษและดิจิทัล เรียนภาคกลาง ทะเบียนบ้านอยู่หาดใหญ่สมัครได้ไหมครับ";
  const context = buildVectorContext(userMessage);
  const answer = askOpenAI({ userMessage, context, history: [] });
  Logger.log(answer);
}
...
function testFileSearchAnswer() {
  const question =
    "สวัสดีครับ แผนภาษาอังกฤษและดิจิทัล เรียนภาคกลาง ทะเบียนบ้านอยู่หาดใหญ่สมัครได้ไหมครับ";
  const answer = askOpenAI({
